...
> **Status:** the v0.1 gate (zero-config DynamoDB `GetItem` + S3
> `GetObject` end-to-end) is met on `feat/next-steps` (PR #7). M1–M4
> sections below still describe what's on `main`; M5–M7 are
> superseded by [`m5-codegen-pivot.md`](m5-codegen-pivot.md) and the
> v0.1 close work tracked in [`../next_steps.md`](../next_steps.md).
> For the current state — wire-format compliance, parity vs
> aws-sdk-rust, remaining v0.2 candidates — see
