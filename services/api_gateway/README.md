# gleam_aws_api_gateway

Typed Gleam client for AWS api gateway. Auto-generated from the
upstream Smithy model in [aws-gleam](https://github.com/Ulberg/aws-gleam).

```gleam
import aws/services/api_gateway

pub fn main() {
  let assert Ok(client) = api_gateway.new_with_auto_region()
  // ... typed ops, e.g. api_gateway.<op>(client, input)
}
```

Depends on [`gleam_aws_runtime`](https://hex.pm/packages/gleam_aws_runtime)
for SigV4 signing, credentials, endpoint resolution, retry, and
the protocol codecs. Each AWS service ships as a separate hex
package so consumers only compile the services they import; the
SDK's full set of ~409 generated services lives at
<https://github.com/Ulberg/aws-gleam/tree/main/services>.

## Documentation

Full docs at <https://hexdocs.pm/gleam_aws_api_gateway>.

## License

Apache 2.0. See LICENSE.
